...
public class AutoTrash implements CommandExecutor {
    ...
    public AutoTrash(SaveCore plugin) {
        this.plugin = plugin;
        ...
        this.NO_PERMISSION = this.plugin.getLangFile().getString("Messages.NO_PERMISSION");
            ...
            if (p.hasPermission("autotrash.use") || p.hasPermission("autotrash.admin"))
                if (args.length == 0) {
                    for (String line : this.HELP_PAGE)
                        p.sendMessage(SaveCore.color(line).replace("%label%", c.getLabel()));
                } else if (args[0].equalsIgnoreCase("add")) {
                    if (args.length == 2) {
                        Material item = Material.getMaterial(args[1].toUpperCase());
                        if (item == null) {
                            p.sendMessage(SaveCore.color(this.NOT_VALID_ITEM));
                        } else {
                            this.plugin.addAutoTrashItem(p, item);
                            p.sendMessage(SaveCore.color(this.ITEM_ADD_SUCCESS.replace("%item%", item.name())));
                        }
                    } else {
                        p.sendMessage(SaveCore.color(this.NOT_ENOUGH_ARGS));
                    }
                    ...
                    if (args.length == 2) {
                        ...
                        if (item == null) {
                            ...
                            p.sendMessage(SaveCore.color(this.ITEM_REM_SUCCESS.replace("%item%", item.name())));
                        }
                    } else {
                        p.sendMessage(SaveCore.color(this.NOT_ENOUGH_ARGS));
                    }
                    ...
                    if (this.plugin.getTrashItems(p) == null) {
                        p.sendMessage(SaveCore.color(this.EMPTY_TRASH_LIST));
                        return false;
                    }
                    List<String> items = new ArrayList<>(Objects.requireNonNull(this.plugin.getTrashItems(p)));
                    String pl = String.join(", ", items);
                    p.sendMessage(SaveCore.color(this.TRASH_LIST_FORMAT.replace("%trashlist%", pl)));
                } else if (args[0].equalsIgnoreCase("reset")) {
                    this.plugin.resetTrashItems(p);
                    p.sendMessage(SaveCore.color(this.TRASH_LIST_RESET));
                } else if (args[0].equalsIgnoreCase("reload")) {
                    if (p.hasPermission("autotrash.admin")) {
                        this.plugin.reloadConfig();
                        p.sendMessage(SaveCore.color(this.RELOAD_MESSAGE));
                    } else {
                        p.sendMessage(SaveCore.color(this.NO_PERMISSION));
                    }
                } else {
                    p.sendMessage(SaveCore.color(this.NOT_VALID_ARG));
                }
            return false;
        }
